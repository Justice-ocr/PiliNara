  ...
  return Obx(
    () {
      ...
      try {
        ...
        if (data == null) {
          final width = desktopSeekPreviewWidth(plPlayerController, maxHeight);
          return positionPreview(
            _SeekPreviewPlaceholder(width: width, height: height),
            width,
          );
        }
        if (data.image.isEmpty) return const SizedBox.shrink();
        ...
        return positionPreview(
          Obx(
            () {
              ...
              if (index == null) {
                return _SeekPreviewPlaceholder(width: width, height: height);
              }
              ...
              return ClipRRect(
                ...
                child: VideoShotImage(
                  ...
                  onSetSize: (xSize, ySize) {
                    if (imgXSize == xSize && imgYSize == ySize) return;
                    data
...
double desktopSeekPreviewHeight(
  ...
  final scale =
      plPlayerController.isFullScreen.value &&
          (Platform.isWindows || !plPlayerController.isVertical)
      ? 4
      : 3;
  var height = 27.0 * scale;
  final compatibleHeight = maxHeight - 140;
  if (compatibleHeight > 50) {
    height = math.min(height, compatibleHeight);
  }
  return height;
}
...
double desktopSeekPreviewWidthFromSize({
  ...
  if (imgXSize <= 0 || imgYSize <= 0) return height * 16 / 9;
...
class _SeekPreviewPlaceholder extends StatelessWidget {
  const _SeekPreviewPlaceholder({required this.width, required this.height});
